...
utf8.length = function utf8_length(string) {
    ...
    for (var i = 0; i < string.length; ++i) {
        ...
        else if ((c & 0xFC00) === 0xD800 && (string.charCodeAt(i + 1) & 0xFC00) === 0xDC00) { ++i; len += 4; }
        else len += 3;
    }
    return len;
};
...
utf8.read = function utf8_read(buffer, start, end) {
    ...
    while (start < end) {
        ...
        if (t <= 0x7F) {
            ...
            chunk[i++] = c2 >= 0x80 ? c2 : REPLACEMENT;
        } else if (t >= 0xE0 && t < 0xF0) {
            c3 = (t & 0xF) << 12 | (buffer[start++] & 0x3F) << 6 | buffer[start++] & 0x3F;
            chunk[i++] = c3 >= 0x800 ? c3 : REPLACEMENT;
        } else if (t >= 0xF0) {
...
for (var b64i = 0; b64i < 64;)
    s64[b64[b64i] = b64i < 26 ? b64i + 65 : b64i < 52 ? b64i + 71 : b64i < 62 ? b64i - 4 : b64i - 59 | 43] = b64i++;

base64.encode = function b64_encode(buffer, start, end) {
...
base64.decode = function b64_decode(string, buffer, offset) {
    var start = offset;
    var j = 0, t;
...
base64.test = function b64_test(string) {
...
var float = (function buildFloat() {
    ...
    if (typeof Float32Array !== "undefined") {
        ...
        function writeFloat_f32_cpy(val, buf, pos) {
            f32[0] = val;
            buf[pos] = f8b[0]; buf[pos + 1] = f8b[1]; buf[pos + 2] = f8b[2]; buf[pos + 3] = f8b[3];
        }
        function writeFloat_f32_rev(val, buf, pos) {
            f32[0] = val;
            buf[pos] = f8b[3]; buf[pos + 1] = f8b[2]; buf[pos + 2] = f8b[1]; buf[pos + 3] = f8b[0];
        }
        function readFloat_f32_cpy(buf, pos) {
            f8b[0] = buf[pos]; f8b[1] = buf[pos + 1]; f8b[2] = buf[pos + 2]; f8b[3] = buf[pos + 3];
            return f32[0];
        }
        function readFloat_f32_rev(buf, pos) {
            f8b[3] = buf[pos]; f8b[2] = buf[pos + 1]; f8b[1] = buf[pos + 2]; f8b[0] = buf[pos + 3];
            return f32[0];
        }
        f.writeFloatLE = le ? writeFloat_f32_cpy : writeFloat_f32_rev;
        f.writeFloatBE = le ? writeFloat_f32_rev : writeFloat_f32_cpy;
        ...
        f.writeFloatLE = writeFloat_ieee754.bind(null, writeUintLE);
        f.writeFloatBE = writeFloat_ieee754.bind(null, writeUintBE);
        f.readFloatLE = readFloat_ieee754.bind(null, readUintLE);
        f.readFloatBE = readFloat_ieee754.bind(null, readUintBE);
    }
    ...
    if (typeof Float64Array !== "undefined") {
        ...
        function writeDouble_f64_cpy(val, buf, pos) {
            f64[0] = val;
            buf[pos] = d8b[0]; buf[pos + 1] = d8b[1]; buf[pos + 2] = d8b[2]; buf[pos + 3] = d8b[3];
            buf[pos + 4] = d8b[4]; buf[pos + 5] = d8b[5]; buf[pos + 6] = d8b[6]; buf[pos + 7] = d8b[7];
        }
        function writeDouble_f64_rev(val, buf, pos) {
            f64[0] = val;
            buf[pos] = d8b[7]; buf[pos + 1] = d8b[6]; buf[pos + 2] = d8b[5]; buf[pos + 3] = d8b[4];
            buf[pos + 4] = d8b[3]; buf[pos + 5] = d8b[2]; buf[pos + 6] = d8b[1]; buf[pos + 7] = d8b[0];
        }
        function readDouble_f64_cpy(buf, pos) {
            d8b[0] = buf[pos]; d8b[1] = buf[pos + 1]; d8b[2] = buf[pos + 2]; d8b[3] = buf[pos + 3];
            d8b[4] = buf[pos + 4]; d8b[5] = buf[pos + 5]; d8b[6] = buf[pos + 6]; d8b[7] = buf[pos + 7];
            return f64[0];
        }
        function readDouble_f64_rev(buf, pos) {
            d8b[7] = buf[pos]; d8b[6] = buf[pos + 1]; d8b[5] = buf[pos + 2]; d8b[4] = buf[pos + 3];
            d8b[3] = buf[pos + 4]; d8b[2] = buf[pos + 5]; d8b[1] = buf[pos + 6]; d8b[0] = buf[pos + 7];
            return f64[0];
        }
        f.writeDoubleLE = dle ? writeDouble_f64_cpy : writeDouble_f64_rev;
        f.writeDoubleBE = dle ? writeDouble_f64_rev : writeDouble_f64_cpy;
        f.readDoubleLE = dle ? readDouble_f64_cpy : readDouble_f64_rev;
        f.readDoubleBE = dle ? readDouble_f64_rev : readDouble_f64_cpy;
    } else {
        ...
        f.writeDoubleLE = writeDouble_ieee754.bind(null, writeUintLE, 0, 4);
        f.writeDoubleBE = writeDouble_ieee754.bind(null, writeUintBE, 4, 0);
        f.readDoubleLE = readDouble_ieee754.bind(null, readUintLE, 0, 4);
        f.readDoubleBE = readDouble_ieee754.bind(null, readUintBE, 4, 0);
    }

    function writeUintLE(val, buf, pos) {
        buf[pos] = val & 255; buf[pos + 1] = val >>> 8 & 255;
        buf[pos + 2] = val >>> 16 & 255; buf[pos + 3] = val >>> 24;
    }
    function writeUintBE(val, buf, pos) {
        buf[pos] = val >>> 24; buf[pos + 1] = val >>> 16 & 255;
        buf[pos + 2] = val >>> 8 & 255; buf[pos + 3] = val & 255;
    }
...
LongBits.prototype.toHash = function toHash() {
    return String.fromCharCode(
        this.lo & 255, this.lo >>> 8 & 255, this.lo >>> 16 & 255, this.lo >>> 24,
        this.hi & 255, this.hi >>> 8 & 255, this.hi >>> 16 & 255, this.hi >>> 24
    );
};
...
util.asPromise = function asPromise(fn, ctx) {
    var params = new Array(arguments.length - 1),
        offset = 0, index = 2, pending = true;
...
var createWriter = function createWriter() {
    return util.Buffer
...
protobuf.BufferWriter = BufferWriter;
protobuf.Reader = Reader;
protobuf.BufferReader = BufferReader;

protobuf.util = util;
protobuf.rpc = { Service: Service };
protobuf.roots = {};
