// Vendored crate (crates.io rusty_enet 0.4.0 + the decentraland-pulse constant patch). Cap its own
// warnings: a registry dep gets `--cap-lints allow`, but a `[patch]` path dep does not, so the
// workspace's RUSTFLAGS=-Dwarnings gate would otherwise fail on this transpiled-from-C source.
#![allow(warnings)]
