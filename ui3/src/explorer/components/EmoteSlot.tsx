...
export default function EmoteSlot({
  thumbnail,
  rarity,
  number,
  width = 174,
  ...
  const spec = RARITIES[rarity ?? "base"] ?? RARITIES.base!;
  ...
  return (
    ...
    >
      <svg className="es__bg" viewBox={`0 0 ${VB_W} ${VB_H}`} preserveAspectRatio="none">
        <defs>
          {spec.glows.map((g, i) => (
            <filter
              key={i}
      ...
      {number != null && (
        <span className="es__num" style={numStyle}>
          {number}
        </span>
      )}
    </div>
